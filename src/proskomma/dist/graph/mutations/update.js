...
var inputItemObject = require('../queries/inputItemObject');
...
var updateMutations = {
  updateItems: {
    ...
    resolve: (root, args) => {
      ...
      if (!docSet) {
        throw new Error("DocSet '".concat(args.docSetId, "' not found"));
      }
      ...
      return docSet.updateItems(args.documentId, args.sequenceId, args.blockPosition, args.items || aghast2items(string2aghast(args.aghast)));
    }
  },
  gcSequences: {
    type: GraphQLNonNull(GraphQLBoolean),
    ...
    resolve: (root, args) => {
      ...
      var document = root.documents[args.documentId];

      if (!document) {
        throw new Error("Document '".concat(args.documentId, "' not found"));
      }
