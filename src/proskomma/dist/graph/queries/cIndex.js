...
var cIndexType = new GraphQLObjectType({
  name: 'cIndex',
  ...
  fields: () => ({
    ...
    items: {
      ...
      resolve: (root, args, context) => context.docSet.itemsByIndex(context.doc.sequences[context.doc.mainId], root[1], args.includeContext).reduce((a, b) => a.concat([['token', 'lineSpace', ' ']].concat(b)), [])
    },
    dumpItems: {
      type: GraphQLNonNull(GraphQLString),
      description: 'The items as a string in a compact eyeballable format',
      resolve: (root, args, context) => {
        var items = context.docSet.itemsByIndex(context.doc.sequences[context.doc.mainId], root[1], args.includeContext);
    ...
    tokens: {
      ...
      resolve: (root, args, context) => context.docSet.itemsByIndex(context.doc.sequences[context.doc.mainId], root[1], args.includeContext).reduce((a, b) => a.concat([['token', 'lineSpace', ' ']].concat(b)), []).filter(i => i[0] === 'token' && (!args.withChars || args.withChars.includes(i[2])))
    },
    text: {
      type: GraphQLNonNull(GraphQLString),
      description: 'The text of the chapter as a single string',
      resolve: (root, args, context) => context.docSet.itemsByIndex(context.doc.sequences[context.doc.mainId], root[1]).reduce((a, b) => a.concat([['token', 'lineSpace', ' ']].concat(b)), []).filter(i => i[0] === 'token').map(t => t[1] === 'lineSpace' ? ' ' : t[2]).join('').trim()
    }
  })
});
