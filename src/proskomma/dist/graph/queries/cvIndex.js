...
var verseNumberType = require('./verseNumber');

var verseRangeType = require('./verseRange');

var cvVerseElementType = new GraphQLObjectType({
  name: 'cvVerseElement',
  ...
  fields: () => ({
    ...
    items: {
      ...
      resolve: (root, args, context) => context.docSet.itemsByIndex(context.doc.sequences[context.doc.mainId], root, args.includeContext).reduce((a, b) => a.concat([['token', 'lineSpace', ' ', null]].concat(b)))
    },
    dumpItems: {
      type: GraphQLNonNull(GraphQLString),
      description: 'The items as a string in a compact eyeballable format',
      resolve: (root, args, context) => dumpItems(context.docSet.itemsByIndex(context.doc.sequences[context.doc.mainId], root, args.includeContext).reduce((a, b) => a.concat([['token', 'lineSpace', ' ', null]].concat(b))))
    },
    tokens: {
      type: GraphQLNonNull(GraphQLList(itemType)),
      ...
      resolve: (root, args, context) => context.docSet.itemsByIndex(context.doc.sequences[context.doc.mainId], root, args.includeContext).reduce((a, b) => a.concat([['token', 'lineSpace', ' ', null]].concat(b))).filter(i => i[0] === 'token' && (!args.withChars || args.withChars.includes(i[2])))
    },
    text: {
      type: GraphQLNonNull(GraphQLString),
      description: 'The text of the verse as a single string',
      resolve: (root, args, context) => context.docSet.itemsByIndex(context.doc.sequences[context.doc.mainId], root).reduce((a, b) => a.concat([['token', 'lineSpace', ' ', null]].concat(b))).filter(i => i[0] === 'token').map(t => t[1] === 'lineSpace' ? ' ' : t[2]).join('').trim()
    }
  })
});
var cvVersesType = new GraphQLObjectType({
  name: 'cvVerses',
...
var cvIndexType = new GraphQLObjectType({
  name: 'cvIndex',
  ...
  fields: () => ({
    ...
    verseRanges: {
      ...
      resolve: root => {
        ...
        for (var [vn, vo] of [...root[1].entries()].filter(v => v[1].length > 0)) {
