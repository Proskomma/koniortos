...
var blockType = require('./block');

var itemGroupType = require('./itemGroup');

var inputAttSpecType = require('./input_att_spec');
...
var blockHasAtts = (docSet, block, attSpecsArray, attValuesArray, requireAll) => {
  var matched = new Set([]);

  for (var item of docSet.unsuccinctifyPrunedItems(block, options, false)) {
    var [att, attType, element, key, count, value] = item[2].split('/');

    for (var [n, attSpecs] of attSpecsArray.entries()) {
      for (var attSpec of attSpecs) {
...
var sequenceType = new GraphQLObjectType({
  name: 'Sequence',
  ...
  fields: () => ({
    ...
    blocks: {
      ...
      resolve: (root, args, context) => {
        ...
        if (args.withChars) {
          ...
          var charsIndexesArray = exactSearchTermIndexes(context.docSet, args.withChars, args.allChars);

          var _loop = function _loop(charsIndexes) {
        ...
        if (args.withMatchingChars) {
          ...
          var _charsIndexesArray = regexSearchTermIndexes(context.docSet, args.withMatchingChars, args.allChars);

          var _loop2 = function _loop2(_charsIndexes) {
            ret = ret.filter(b => context.docSet.blockHasChars(b, _charsIndexes));
          };

          for (var _charsIndexes of _charsIndexesArray) {
            _loop2(_charsIndexes);
          }
        }
    ...
    wordLikes: {
      ...
      resolve: (root, args, context) => {
        if (root.type !== 'main') {
          throw new Error("Only available for the main sequence, not ".concat(root.type));
        }
        ...
        for (var b of root.tokensPresent) {
          if (b) {
            var enumOffset = context.docSet.enumIndexes['wordLike'][n];
            var tokenString = context.docSet.enums['wordLike'].countedString(enumOffset);
