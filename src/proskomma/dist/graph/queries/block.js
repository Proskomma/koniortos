...
var blockType = new GraphQLObjectType({
  name: 'Block',
  ...
  fields: () => ({
    ...
    os: {
      ...
      resolve: (root, args, context) => context.docSet.unsuccinctifyScopes(root.os)
    },
    bs: {
      type: GraphQLNonNull(itemType),
      description: 'The block scope for this block',
      resolve: (root, args, context) => {
        var [itemLength, itemType, itemSubtype] = headerBytes(root.bs, 0);
    ...
    bg: {
      ...
      resolve: (root, args, context) => context.docSet.unsuccinctifyGrafts(root.bg)
    },
    nt: {
      type: GraphQLNonNull(GraphQLInt),
      description: 'The value of nextToken at the start of this block',
      resolve: root => root.nt.nByte(0)
    },
    items: {
      type: GraphQLNonNull(GraphQLList(GraphQLNonNull(itemType))),
    ...
    aghast: {
      ...
      resolve: (root, args, context) => {
        var itemArrays = context.docSet.unsuccinctifyItems(root.c, {}, null);
        var items = itemArrays2Objects(itemArrays);
        var aghast = items2aghast(items);
        var aghString = aghast2string(aghast);
        return aghString;
      }
    },
    tokens: {
      ...
      resolve: (root, args, context) => {
        ...
        if (args.withScriptureCV) {
          ret = context.docSet.unsuccinctifyItemsWithScriptureCV(root, args.withScriptureCV, {
            tokens: true
        ...
        if (args.withChars) {
          ...
          ret = ret.filter(i => {
            for (var re of args.withMatchingChars) {
              if (_xregexp.default.test(i, (0, _xregexp.default)(re))) {
                return true;
              }
            }
    ...
    text: {
      ...
      resolve: (root, args, context) => {
        var tokens = args.withScriptureCV ? context.docSet.unsuccinctifyItemsWithScriptureCV(root, args.withScriptureCV, {
          tokens: true
        }, false) : context.docSet.unsuccinctifyItems(root.c, {
          tokens: true
        }, null);
        var ret = tokens.map(t => t[2]).join('').trim();
    ...
    dump: {
      ...
      resolve: (root, args, context) => dumpBlock(context.docSet.unsuccinctifyBlock(root, {}, null))
    },
    scopeLabels: {
      type: GraphQLNonNull(GraphQLList(GraphQLNonNull(GraphQLString))),
      description: 'A list of the labels for the block\'s bs, os and is scopes',
      resolve: (root, args, context) => [...context.docSet.unsuccinctifyBlockScopeLabelsSet(root)]
    }
  })
});
