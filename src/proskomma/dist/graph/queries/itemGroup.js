...
var itemType = require('./item'); // [scopeLabels, items]


var itemGroupType = new GraphQLObjectType({
  name: 'ItemGroup',
  ...
  fields: () => ({
    ...
    tokens: {
      ...
      resolve: (root, args, context) => root[1].filter(i => i[0] === 'token' && (!args.withChars || args.withChars.includes(i[2])))
    },
    text: {
      ...
      resolve: (root, args, context) => {
        var tokensText = root[1].filter(i => i[0] === 'token').map(t => t[2]).join('');
