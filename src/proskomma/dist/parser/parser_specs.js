...
} = require('./lexers/object_for_fragment');

var buildSpecLookup = specs => {
  var ret = {};

  for (var spec of specs) {
    for (var context of spec.contexts) {
      if (!(context[0] in ret)) {
        ret[context[0]] = {};
      }

      var accessor = context[1];
      ...
      if (!accessor) {
        ...
        for (var accessorValue of context[2]) {
...
var specs = pt => [{
  ...
  contexts: [['startTag', 'tagName', ['id', 'usfm', 'ide', 'sts', 'h', 'toc', 'toca']]],
  parser: {
      ...
      onEnd: (parser, label) => {
        ...
        if (label === 'id') {
          if (parser.headers[label].length === 3 || parser.headers[label].length > 3 && parser.headers[label].substring(3, 4) === ' ') {
            var bookCode = parser.headers[label].substring(0, 3);
  ...
  contexts: [['startTag', 'tagName', ['ms', 'mr', 's', 'sr', 'r', 'qa', 'sp', 'sd'].concat(pt.customTags.heading)]],
  parser: {
  ...
  contexts: [['startTag', 'tagName', ['imt', 'is', 'ip', 'ipi', 'im', 'imi', 'ipq', 'imq', 'ipr', 'iq', 'ib', 'ili', 'iot', 'io', 'iex', 'imte'].concat(pt.customTags.intro)]],
  parser: {
  ...
  contexts: [['startTag', 'tagName', ['cd', 'p', 'm', 'po', 'pr', 'cls', 'pmo', 'pm', 'pmc', 'pmr', 'pi', 'mi', 'nb', 'pc', 'ph', 'b', 'q', 'qr', 'qc', 'qa', 'qm', 'qd', 'lh', 'li', 'lf', 'lim', 'd', 'zlexorth', 'zlexbrief', 'zlexfull'].concat(pt.customTags.paragraph)]],
  parser: {
  ...
  contexts: [['startTag', 'tagName', ['qs', 'qac', 'litl', 'lik', 'liv', 'fr', 'fq', 'fqa', 'fk', 'fl', 'fw', 'fp', 'fv', 'ft', 'fdc', 'fm', 'xo', 'xk', 'xq', 'xt', 'xta', 'xop', 'xot', 'xnt', 'xdc', 'rq', 'add', 'bk', 'dc', 'k', 'nd', 'ord', 'pn', 'png', 'qt', 'sig', 'sls', 'tl', 'wj', 'em', 'bd', 'it', 'bdit', 'no', 'sc', 'sup', 'ior', 'iqt'].concat(pt.customTags.char)]],
  parser: {
    newScopes: [{
  ...
  contexts: [['startTag', 'tagName', ['w', 'rb' // 'xt',
